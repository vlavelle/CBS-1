...
shinyServer(function(input, output) {
  ...
  data_84709 <- reactive({
    data84709 %>%
      ...
      mutate(tooltip_text = paste0("Feature: ", Feature, "\n",
  ...
  dataDrivingLicense1 <- reactive(
    data83488%>%
      ...
      mutate(tooltip_text = paste0("Count: ", PeopleWithADrivingLicence_1, "\n",
                                   "Region: ", Region, "\n"))
  )
  ...
  output$DrivingLicense2 <- renderPlotly({
    DrivingLicense2 <- ggplot(dataDrivingLicense2(), 
                              aes(x= AgeDrivingLicenseHolder, 
                                  y = PeopleWithADrivingLicence_1, 
                                  fill = Region,
                                  text = tooltip_text)) + 
      geom_col(position = position_dodge()) +
  ...
  dataFueltypes2 <- reactive(
    datafueltypes1 %>%
      ...
      mutate(tooltip_fuel = paste0("Count: ", Count, "\n",
  ...
  data_83712 <- data83712 %>%
    mutate(tooltip_traffic = paste0("Region: ", provinces, "\n",
